...
from sdc.config import xycols, processors
...
size = data.DataSet(config.train_data).size()
# split to train, validation and test
trainset, others = dataset.split(test_size = int(size / 5))
valset, testset = others.split(test_size = int(size / 10))
