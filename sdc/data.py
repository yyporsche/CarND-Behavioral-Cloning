...
class DataSet(object):
	...
	def add_side_image(self):
		...
		side["CenterImage"] = side["LeftImage"]
		side["SteeringAngle"] = side["SteeringAngle"].astype(np.float32) + 0.25
		self.log = pd.concat([self.log, side], axis=0, ignore_index=True)
		
		side["CenterImage"] = side["RightImage"]
		side["SteeringAngle"] = side["SteeringAngle"].astype(np.float32) - 0.25
		self.log = pd.concat([self.log, side], axis=0, ignore_index=True)
	...
	def remove_noise(self):
		...
		print("%d percent of records have been removed due to speed <= 20" % ((N- self.log.shape[0])/N*100))
		return self
